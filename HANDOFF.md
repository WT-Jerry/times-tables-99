...
- 快取：`styles.css?v=2`、`game.css?v=6`、`shell.js?v=2`、`app.js?v=3`、`select.js?v=3`、`play.js?v=10`。改檔要再加版本。
...
- 十題全對：先隨機播 `assets/win-clip-yellow.mp4` 或 `assets/win-clip-white.mp4`（約 4 秒，他附的兩支動畫；第三支是重複）。播完淡進結果頁，標題上方再隨機放狗狗。進測驗才開始載影片，不要在主頁先抓。音效開時暫停背景音樂、不歸零，播完接回；音效關則影片靜音。播不起來不要卡住，直接進結果。不是全對不播。
