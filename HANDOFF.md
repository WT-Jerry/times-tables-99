...
- 快取：`styles.css?v=4`、`game.css?v=10`、`shell.js?v=3`、`app.js?v=4`、`select.js?v=3`、`play.js?v=11`、`table.js?v=2`。改檔要再加版本。
...
- 十題全對：先隨機播 `assets/win-clip-yellow.mp4` 或 `assets/win-clip-white.mp4`（約 4 秒）。手機要先靜音 `play()`，畫面真的往前走才開聲音、才開始計時。不要在 1px／透明盒子裡呼叫 `play()`，也不要等 `play()` 的 Promise 就當它在播。播完淡進結果頁。
- 主頁「99乘法表」和「玩」在底部左右並排，狗狗要留在按鈕上方，不准壓到。
- 背誦頁 `?view=table&n=1`…`9`。上方 1～9 圓鈕切換，點幾就顯示那個號碼的乘法表。奶油卡片、兩排白色算式膠囊，背景用主頁森林。底部「回關卡地圖」。不要把 Jerry 的備註句放進頁面。關卡遊戲這回先不接。
