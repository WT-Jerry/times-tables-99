...
- 快取：`styles.css?v=4`、`game.css?v=11`、`shell.js?v=3`、`app.js?v=4`、`select.js?v=3`、`play.js?v=12`、`table.js?v=2`。改檔要再加版本。
...
- 十題全對：隨機播黃或白的慶祝動畫約 4 秒，再淡進結果頁。iPhone 會把 `<video>` 停在第一幀，不要再靠 `video.play()`。用 `assets/win-clip-*-sheet.jpg`（8×4、32 格）畫到 canvas 換格，聲音用同名 `.m4a`。進測驗就先載這兩張圖。
