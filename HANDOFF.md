...
- 快取：`styles.css?v=2`、`game.css?v=5`、`shell.js?v=1`、`app.js?v=3`、`select.js?v=3`、`play.js?v=8`。改檔要再加版本。
...
- 答錯：跳出「記住這個算式」，答案綠色。隨機 `assets/review-yellow.png` 或 `assets/review-white.png`。按右邊 `>` 才下一題。
- **狗狗必須整隻留在藍卡白框裡面**，禁止負邊距壓到白框（Jerry 已打回）。
- 答對：綠一下後自動下一題。
- 結束：答對幾題、答錯幾題；「再考一次」或「回關卡地圖」。
- 十題全對：紅框那塊空白（「測驗結果」上方）隨機放 `assets/win-yellow.png` 或 `assets/win-white.png`，左右置中，不准壓到標題或分數。進測驗就先載圖，圖解碼完才跟文字一起顯示。不是全對不要放。
